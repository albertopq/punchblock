      ...
      module Translator
        class Asterisk < GenericConnection
          ...
          def write(cmd)
            ...
            call_server = call_server_for_id call_id
            call_server.send_data ozone_to_agi(cmd)
            response = call_server.event_queue.pop
            n = agi_to_ozone(call_id, cmd, response)
          ...
          def agi_to_ozone(call_id, cmd, response)
            if response =~ /$200/
              Event::Complete.new.tap do |c|
                c.call_id = call_id
                c.original_command = cmd
                c.connection = self
              end
            end
          end
